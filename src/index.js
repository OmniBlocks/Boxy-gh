      ...
      if (pendingTasks.length > 0) {
        ...
        if (bgContext) {
          ...
          const systemPrompt = `
            ...
            4. To actually open a pull request once you've committed and pushed a branch, you MUST use the 'create_pull_request' tool. Do NOT run 'gh pr create' inside execute_command and do NOT tell anyone a PR was opened based on shell output, because that output is easy to misread and it's not proof anything happened. 'create_pull_request' talks to the GitHub API directly, targets the exact repo this task came from, and gives you a PR number and URL on success, or the exact GitHub error on failure. Only ever report a pull request as created if this tool returns status 'success'.
