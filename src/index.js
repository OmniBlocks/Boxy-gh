      ...
      if (pendingTasks.length > 0) {
        ...
        if (bgContext) {
          ...
          const systemPrompt = `
            ...
            2. You have access to a computer to run commands. You can use it to run shell commands, scripts, or any other command-line tools you need. Use this to help you complete the task. You can also use it to check the state of the repository or run git commands (clone, branch, add, commit, push). This is a very RAM-constrained (256mb) alpine linux environment, so only use this for file editing or committing/pushing, not for running any tests or building projects (like pnpm run build or pnpm test). Do not use it for anything that creates a constant stream like dev servers or watchers since it will just time you out. The container is 100% persistent. Run commands using execute_command.
            3. To actually open a pull request once you've committed and pushed a branch, you MUST use the 'create_pull_request' tool. Do NOT run 'gh pr create' inside execute_command and do NOT tell anyone a PR was opened based on shell output, because that output is easy to misread and it's not proof anything happened. 'create_pull_request' talks to the GitHub API directly, targets the exact repo this task came from, and gives you a PR number and URL on success, or the exact GitHub error on failure. Only ever report a pull request as created if this tool returns status 'success'.
            4. Use 'create_comment' to report your findings on the relevant issue. Make sure to read the issue or PR first to understand the context of the conversation before commenting, so it's not awkward or out of context, and you know exactly what you said before. On issue threads, you are pinged as @OmniBlocks/boxy, but your username is boxycpu[bot].
            5. When you are entirely done, call 'complete_todo_list_item' with id '${taskId}'.
  ...
  if (cleanedComment === mentionHandle) {
    ...
    try {
      ...
      const systemPrompt = `
        ...
        - Command Execution: You have your own computer to run linux commands!!!!! You can use this for whatever you need in a task, including git clone/branch/commit/push. It runs alpine linux with 256mb of ram. For more info, check your computer's manual (shouldn't be necessary though)
